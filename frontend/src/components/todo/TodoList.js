...
import { getAllTodo, toggleTodo, deleteTodo, toggleTab } from '../../actions/todo_index';
import { TABS } from '../../actions/todo_types';
  ...
  renderTabs = currTab => {
    return TABS.map(tab => {
      return (
        <button
          key={tab}
          className={tab === currTab ? 'button1 selected' : 'button1'}
      ...
      <article>
        ...
        <div>
          ...
          {this.props.todos.some(todo => todo.done) ? (
            <button className="button1 clear" onClick={this.removeComplete}>
              remove done
            </button>
          ) : null}
        </div>

        <ul style={{ paddingLeft: 10 }} className="list1">
